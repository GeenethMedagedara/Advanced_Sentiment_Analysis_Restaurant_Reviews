...
class BERTSentimentAnalysisStrategy(ABSAStrategy):
    ...
    def train(self, train_data: pd.DataFrame, val_data: pd.DataFrame, epochs: int = 4, learning_rate: float = 5e-5):
        ...
        try:
            ...
            # Check if an MLflow run is already active before starting a new one
            ...
            mlflow.transformers.log_model(
                transformers_model=save_path,
                artifact_path="ABSA_model",
                task="text-classification"
            )
            print(f"Model saved at {save_path}")

            # Only end the run if we started it in this function
